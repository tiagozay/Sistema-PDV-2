<?php
    ...
    class PdoVendaRepository
    {
        ...
        {            
            ...
            if($ordem == 'mais_antiga' || is_null($ordem)){
                $stmt = $this->conexao->query("SELECT 
                    ...
                    c.cpf, 
                    c.nome  
                    FROM vendas v LEFT JOIN cliente c ON v.cliente = c.id
                ");
        ...
        public function venda_com_produtos(?int $id)
        {
            ...
            $stmt_produtos = $this->conexao->query("SELECT * FROM produtos_venda WHERE venda = {$venda->getId()}");
        ...
        public function vendas_do_cliente(Cliente $cliente): array
        {
            $stmt = $this->conexao->query("SELECT 
                ...
                c.cpf, 
                c.nome  
                FROM vendas v LEFT JOIN cliente c ON v.cliente = c.id WHERE v.cliente = {$cliente->getId()}"
            );
        ...
        public function save(Venda $venda) : bool
        {
            if(is_null($venda->getId())){
                return $this->cadastra_venda($venda);
            }
        }

        private function cadastra_venda(Venda $venda)
        {
            $data = $this->dataAtual();

            $id_cliente = is_null($venda->getCliente()) ? 'null' : $venda->getCliente()->getId();

            $insert = $this->conexao->exec("INSERT INTO vendas (
                    data_registro, 
                    qtde_itens, 
                    valor_total, 
                    valor_com_desconto, 
        ...
        {
            return $this->conexao->exec("DELETE FROM vendas WHERE id = {$id}");
        }

        public function remove_cliente(Venda $venda): bool
        {
            $stmt = $this->conexao->query("UPDATE vendas SET cliente = null where id = {$venda->getId()}");
